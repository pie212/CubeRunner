...
public class PlayerNetworkedMovement : NetworkBehaviour
{
    ...
    void FixedUpdate()
    {
        if (rb.velocity.y > -8)
        {
            rb.velocity = new Vector3(rb.velocity.x, rb.velocity.y - ExtraGravityAmount, rb.velocity.z);
        }
        ...
        if (UseRelativeForwardforce == false)
        {
            ...
            rb.velocity = gameObject.transform.forward * forwardForce * Time.deltaTime * rb.mass;
        }
        else
        {
